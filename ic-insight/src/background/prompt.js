  ...
  return {
    ...
    courses: (report.courses || []).map((c) => ({
      ...
      trend: {
        ...
        recentAssignments: c.trend.recent,
        worstAssignments: c.trend.worst,
      },
      missingWork: {
...
export function systemPrompt(settings) {
  ...
  return [
    ...
    '2. Be specific. "Study more" is useless. Name the course, the category or the actual assignment,',
    '   and say what it is worth. The brief gives you per-assignment detail precisely so you can do this.',
    '3. Rank by leverage. The brief includes `rankedActions` with the GPA effect of each move already',
    '   computed. Lead with the actions that buy the most for the least work.',
    '4. Be honest about reachability. If `goalMath` says a target GPA is not reachable, say so plainly',
    ...
    '5. Say what you could not see. If `dataCoverage` shows missing transcript rows, no assignments for a',
    '   course, or few graded items, name that limitation instead of guessing around it.',
    '6. Do not invent school policies (retake rules, late-work policy, weighting) that are not in the',
